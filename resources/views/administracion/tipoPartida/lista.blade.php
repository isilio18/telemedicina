...
@section('js_after')
    ...
    <script>
        $('#borrar').on('show.bs.modal', function (event) {
            $("#borrarForm").attr('action','{{ url('/administracion/tipoPartida/eliminar') }}');
    ...
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            ...
            <div class="block-options">
                <button type="button" class="btn-block-option mr-2"><a href="{{ URL::to('administracion/tipoPartida/nuevo') }}"><i class="fa fa-plus mr-1"></i> Nuevo</a></button>
        ...
        <form action="{{ url('/administracion/tipoPartida/lista') }}" method="get">
            <div class="row">
            ...
            <table class="table table-bordered table-striped table-vcenter">
                ...
                <tbody>
                @foreach($tab_tipo_partida as $key => $value)
                    <tr>
                      <!--  <td class="font-w600">{{ $value->id }}</td> -->
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->de_tipo_partida }}</em></td>
                        <td class="d-none d-sm-table-cell">
                            @if ($value->in_activo == true)
                            <a href="{{ url('/administracion/tipoPartida/deshabilitar').'/'. $value->id }}">
                                <i class="fa fa-eye text-done mr-1"></i>
                            </a>
                            @else
                            <a href="{{ url('/administracion/tipoPartida/habilitar').'/'. $value->id }}">
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                                <a href="{{ url('/administracion/tipoPartida/editar').'/'. $value->id }}">
            ...
            {{ $tab_tipo_partida->appends(Request::only(['perPage','q']))->render() }}         
