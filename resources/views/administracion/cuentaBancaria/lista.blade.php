...
@section('js_after')
    ...
    <script>
        $('#borrar').on('show.bs.modal', function (event) {
            $("#borrarForm").attr('action','{{ url('/administracion/cuentaBancaria/eliminar') }}');
    ...
    <div class="block block-rounded block-bordered">
        ...
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            ...
            <div class="block-options">
                <button type="button" class="btn-block-option mr-2"><a href="{{ URL::to('administracion/cuentaBancaria/nuevo') }}"><i class="fa fa-plus mr-1"></i> Nuevo</a></button>
        ...
        <form action="{{ url('/administracion/cuentaBancaria/lista') }}" method="get">
            <div class="row">
            ...
            <table class="table table-bordered table-striped table-vcenter">
                ...
                <tbody>
                @foreach($tab_cuenta_bancaria as $key => $value)
                    <tr>
                        <!--  <td class="font-w600">{{ $value->id }}</td> -->
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->de_banco }}</em></td>
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->nu_cuenta_bancaria }}</em></td>
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->de_cuenta_bancaria }}</em></td>
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->de_tipo_cuenta_bancaria }}</em></td>
                        <td class="d-none d-sm-table-cell">
                            @if ($value->in_activo == true)
                            <a href="{{ url('/administracion/cuentaBancaria/deshabilitar').'/'. $value->id }}">
                                <i class="fa fa-eye text-done mr-1"></i>
                            </a>
                            @else
                            <a href="{{ url('/administracion/cuentaBancaria/habilitar').'/'. $value->id }}">
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                             @if ($value->in_activo == true)                                
                                <a href="{{ url('/administracion/cuentaBancaria/editar').'/'. $value->id }}">
            ...
            {{ $tab_cuenta_bancaria->appends(Request::only(['perPage','q']))->render() }}         
