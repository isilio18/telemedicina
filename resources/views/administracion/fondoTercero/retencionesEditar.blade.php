...
<script>jQuery(function(){ Dashmix.helpers(['maxlength', 'select2', 'rangeslider', 'masked-inputs', 'pw-strength']); });</script>
    ...
    <script>
        ...
        $(function( $ ){
            ...
            var id_tab_retencion = $("#retencion").val();
            $.post("{{ URL::to('administracion/fondoTercero/montoRetencion') }}", {fecha_inicio: fecha_inicio,fecha_fin: fecha_fin,id_tab_retencion:id_tab_retencion,_token: '{{ csrf_token() }}' }, function(data){
            $("#monto").val(data.data);
...
$("#retencion option:selected").each(function () {
id_tab_retencion=$(this).val();
var fecha_inicio = $("#fecha_inicio").val();
var fecha_fin = $("#fecha_fin").val();
    $.post("{{ URL::to('administracion/fondoTercero/montoRetencion') }}", {fecha_inicio: fecha_inicio,fecha_fin: fecha_fin,id_tab_retencion:id_tab_retencion,_token: '{{ csrf_token() }}' }, function(data){
    $("#monto").val(data.data);
...
<div class="content content-full content-boxed">
    ...
    <!-- New Post -->
    <form action="{{ URL::to('administracion/fondoTercero/guardar').'/'.$data->id }}" method="POST">
                                ...
                                <div class="block-content">
                                    ...
                                    <div class="form-group form-row">
                                        <div class="col-2">
                                            ...
                                            <select class="custom-select {!! $errors->has('tipo_documento') ? 'is-invalid' : '' !!}" name="tipo_documento" readonly id="tipo_documento" {{ $errors->has('tipo_documento') ? 'aria-describedby="tipo_documento-error" aria-invalid="true"' : '' }}>
                                                <option value="" >Seleccione...</option>
                                                @foreach($tab_documento as $documento)
                                                <option readonly value="{{ $documento->id }}" {{ $documento->id == $data->id_tab_documento ? 'selected' : '' }}>{{ $documento->de_inicial }}</option>
                                                @endforeach
                                            </select>
            ...
            <div class="block block-themed block-transparent mb-0">
                ...
                <div class="row justify-content-center push">
                    ...
                    <div class="form-group form-row">
                        ...
                        <input type="text" class="form-control {!! $errors->has('monto') ? 'is-invalid' : '' !!}" readonly id="monto" name="monto" placeholder="Monto Disponible..." value="{{ old('monto') }}" {{ $errors->has('monto') ? 'aria-describedby="monto-error" aria-invalid="true"' : '' }}>
                        @if( $errors->has('monto') )
