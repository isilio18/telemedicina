...
<div class="content content-full content-boxed">
    ...
    <!-- New Post -->
    <form action="{{ URL::to('administracion/formularPresupuesto/partida/guardar').'/'.$data->id }}" method="POST">
                    ...
                    <div class="col-md-10">
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select {!! $errors->has('tipo_ingreso') ? 'is-invalid' : '' !!}" name="tipo_ingreso" id="tipo_ingreso" {{ $errors->has('tipo_ingreso') ? 'aria-describedby="tipo_ingreso-error" aria-invalid="true"' : '' }}>
                                ...
                                <option value="{{ $tipo_ingreso->id }}" {{ $tipo_ingreso->id == $data->id_tab_tipo_ingreso ? 'selected' : '' }}>{{ $tipo_ingreso->nu_tipo_ingreso }}-{{ $tipo_ingreso->de_tipo_ingreso }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control {!! $errors->has('monto') ? 'is-invalid' : '' !!}" id="monto" name="monto" placeholder="Monto..." value="{{ empty(old('monto'))? $data->mo_partida : old('monto') }}" {{ $errors->has('monto') ? 'aria-describedby="monto-error" aria-invalid="true"' : '' }}>
                            @if( $errors->has('monto') )
