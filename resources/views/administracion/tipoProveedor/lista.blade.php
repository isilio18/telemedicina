...
@section('js_after')
    ...
    <script>
        $('#borrar').on('show.bs.modal', function (event) {
            $("#borrarForm").attr('action','{{ url('/administracion/tipoProveedor/eliminar') }}');
    ...
    <div class="block block-rounded block-bordered">
        ...
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            <nav class="flex-sm-00-auto ml-sm-3" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item">Tipos de Proveedor</li>
                    <li class="breadcrumb-item active" aria-current="page">Lista</li>
                </ol>
            </nav>
        </div>              
            <div class="block-options">
                <button type="button" class="btn-block-option mr-2"><a href="{{ URL::to('administracion/tipoProveedor/nuevo') }}"><i class="fa fa-plus mr-1"></i> Nuevo</a></button>
        ...
        <form action="{{ url('/administracion/tipoProveedor/lista') }}" method="get">
            <div class="row">
            ...
            <table class="table table-bordered table-striped table-vcenter">
                ...
                <tbody>
                @foreach($tab_tipo_proveedor as $key => $value)
                    <tr>
                        <!--  <td class="font-w600">{{ $value->id }}</td> -->
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->de_tipo_proveedor }}</em></td>
                        <td class="d-none d-sm-table-cell">
                            @if ($value->in_activo == true)
                            <a href="{{ url('/administracion/tipoProveedor/deshabilitar').'/'. $value->id }}">
                                <i class="fa fa-eye text-done mr-1"></i>
                            </a>
                            @else
                            <a href="{{ url('/administracion/tipoProveedor/habilitar').'/'. $value->id }}">
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                               @if ($value->in_activo == true)                                
                                <a href="{{ url('/administracion/tipoProveedor/editar').'/'. $value->id }}">
            ...
            {{ $tab_tipo_proveedor->appends(Request::only(['perPage','q']))->render() }}         
