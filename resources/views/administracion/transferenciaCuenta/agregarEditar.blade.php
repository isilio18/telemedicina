...
<div class="content content-full content-boxed">
    ...
    <!-- New Post -->
    <form action="{{ URL::to('administracion/transferenciaCuenta/guardarAgregar').'/'.$data->id }}" method="POST">
                ...
                <div class="row justify-content-center push">
                    <div class="col-md-10">
                        ...
                        <div class="block block-themed block-rounded block-bordered">
                            ...
                            @else                                
                            <option value="{{ $banco->id }}" {{ $banco->id == $data->id_tab_banco_debito ? 'selected' : '' }}>{{ $banco->de_banco }}</option>                            
                            @endif                                    
                                @endforeach
                            </select>
                            ...
                            <select class="custom-select {!! $errors->has('cuenta_bancaria_debito') ? 'is-invalid' : '' !!}" name="cuenta_bancaria_debito" id="cuenta_bancaria_debito" {{ $errors->has('cuenta_bancaria_debito') ? 'aria-describedby="cuenta_bancaria_debito-error" aria-invalid="true"' : '' }}>
                                <option value="0" data-mo_disponible="0">Seleccione...</option>
                                @foreach($tab_cuenta_bancaria_debito as $cuenta_bancaria)
                            @if( count($errors) > 0 )
                            <option value="{{ $cuenta_bancaria->id }}" data-mo_disponible="{{ $cuenta_bancaria->mo_disponible }}" {{ $cuenta_bancaria->id == old('cuenta_bancaria_debito') ? 'selected' : '' }}>{{ $cuenta_bancaria->nu_cuenta_bancaria }} - {{ $cuenta_bancaria->de_cuenta_bancaria }}</option>
                            @else 
                            <option value="{{ $cuenta_bancaria->id }}" data-mo_disponible="{{ $cuenta_bancaria->mo_disponible }}" {{ $cuenta_bancaria->id == $data->id_tab_cuenta_bancaria_debito ? 'selected' : '' }}>{{ $cuenta_bancaria->nu_cuenta_bancaria }} - {{ $cuenta_bancaria->de_cuenta_bancaria }}</option>
                            @endif                                  
                        ...
                        <div class="form-group">
                            <label for="fecha_transferencia"">Fecha Transferencia</label>        
                             <input type="text" class="js-flatpickr form-control bg-white {!! $errors->has('fecha_transferencia') ? 'is-invalid' : '' !!}" id="fecha_transferencia" name="fecha_transferencia" placeholder="Fecha Transferencia..." locale="es" data-date-format="d-m-Y" value="{{ empty(old('fecha_transferencia'))? $data->fe_transferencia : old('fecha_transferencia') }}" {{ $errors->has('fecha_transferencia') ? 'aria-describedby="fecha_transferencia-error" aria-invalid="true"' : '' }}>
                        ...
                        <div class="block block-themed block-rounded block-bordered">
                            ...
                            @else 
                            <option value="{{ $banco->id }}" {{ $banco->id == $data->id_tab_banco_credito ? 'selected' : '' }}>{{ $banco->de_banco }}</option>   
                            @endif    
                            @endforeach
                            </select>
                            @if( $errors->has('banco_credito') )
                            ...
                            <select class="custom-select {!! $errors->has('cuenta_bancaria_credito') ? 'is-invalid' : '' !!}" name="cuenta_bancaria_credito" id="cuenta_bancaria_credito" {{ $errors->has('cuenta_bancaria_credito') ? 'aria-describedby="cuenta_bancaria_credito-error" aria-invalid="true"' : '' }}>
                                <option value="0" data-mo_salso="0">Seleccione...</option>
                                @foreach($tab_cuenta_bancaria_credito as $cuenta_bancaria)
                            @if( count($errors) > 0 )
                            <option value="{{ $cuenta_bancaria->id }}" data-mo_saldo="{{ $cuenta_bancaria->mo_disponible }}" {{ $cuenta_bancaria->id == old('cuenta_bancaria_credito') ? 'selected' : '' }}>{{ $cuenta_bancaria->nu_cuenta_bancaria }} - {{ $cuenta_bancaria->de_cuenta_bancaria }}</option>
                            @else 
                            <option value="{{ $cuenta_bancaria->id }}" data-mo_saldo="{{ $cuenta_bancaria->mo_disponible }}" {{ $cuenta_bancaria->id == $data->id_tab_cuenta_bancaria_credito ? 'selected' : '' }}>{{ $cuenta_bancaria->nu_cuenta_bancaria }} - {{ $cuenta_bancaria->de_cuenta_bancaria }}</option>
                            @endif                                
                            ...
                            <select class="custom-select {!! $errors->first('error')=='id_tab_retencion' ? 'is-invalid' : '' !!}" name="id_tab_retencion" id="id_tab_retencion" {{ $errors->first('error')=='id_tab_retencion' ? 'aria-describedby="id_tab_retencion-error" aria-invalid="true"' : '' }}>
                                ...
                                <option value="{{ $retencion->id }}" {{ $retencion->id == $data->id_tab_retencion ? 'selected' : '' }}>{{ $retencion->de_retencion }}</option>
                                @endforeach
                            </select>
