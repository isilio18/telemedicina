...
@section('js_after')
    ...
    <script>
        $('#borrar').on('show.bs.modal', function (event) {
            $("#borrarForm").attr('action','{{ url('/administracion/producto/eliminar') }}');
    ...
    <div class="block block-rounded block-bordered">
        <div class="block-header block-header-default">
            ...
            <div class="block-options">
                <button type="button" class="btn-block-option mr-2"><a href="{{ URL::to('administracion/producto/nuevo') }}"><i class="fa fa-plus mr-1"></i> Nuevo</a></button>
        ...
        <form action="{{ url('/administracion/producto/lista') }}" method="get">
            <div class="row">
            ...
            <table class="table table-bordered table-striped table-vcenter">
                ...
                <tbody>
                @foreach($tab_producto as $key => $value)
                    <tr>
                        <!--  <td class="font-w600">{{ $value->id }}</td> -->
                        <td class="d-none d-sm-table-cell">{{ $value->nu_producto }}</td>
                        <td class="d-none d-sm-table-cell"><em class="text-muted">{{ $value->de_producto }}</em></td>
                        <td class="d-none d-sm-table-cell">
                            @if ($value->in_activo == true)
                            <a href="{{ url('/administracion/producto/deshabilitar').'/'. $value->id }}">
                                <i class="fa fa-eye text-done mr-1"></i>
                            </a>
                            @else
                            <a href="{{ url('/administracion/producto/habilitar').'/'. $value->id }}">
                        ...
                        <td class="text-center">
                            <div class="btn-group">
                                <a href="{{ url('/administracion/producto/editar').'/'. $value->id }}">
            ...
            {{ $tab_producto->appends(Request::only(['perPage','q']))->render() }}         
