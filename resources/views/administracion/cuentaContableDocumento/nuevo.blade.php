...
@section('js_after')
    ...
    @if( !empty( old('solicitud')) )

        $("#ruta").html('<option value=""> Seleccione...</option>');
        $.post("{{ URL::to('administracion/cuentaContableDocumento/ruta') }}", { solicitud: {{  old('solicitud') }}, _token: '{{ csrf_token() }}' }, function(data){
            var opcion;
            $.each(data.data, function(i,f) {
                if( f.id == {{ intval( old('ruta')) }}){ opcion = 'selected="selected"';} else{ opcion = ''; }
...
<div class="content content-full content-boxed">
    ...
    <!-- New Post -->
    <form action="{{ URL::to('administracion/cuentaContableDocumento/guardar') }}" method="POST">
                    ...
                    <div class="col-md-10">
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select {!! $errors->has('solicitud') ? 'is-invalid' : '' !!}" name="solicitud" id="solicitud" {{ $errors->has('solicitud') ? 'aria-describedby="solicitud-error" aria-invalid="true"' : '' }}>
                                ...
                                <option value="{{ $solicitud->id }}" {{ $solicitud->id == old('solicitud') ? 'selected' : '' }}>{{ $solicitud->de_solicitud }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select {!! $errors->has('cuenta_gasto') ? 'is-invalid' : '' !!}" name="cuenta_gasto" id="cuenta_gasto" {{ $errors->has('cuenta_gasto') ? 'aria-describedby="cuenta_gasto-error" aria-invalid="true"' : '' }}>
                                ...
                                <option value="{{ $cuenta_gasto->id }}" {{ $cuenta_gasto->id == old('cuenta_gasto') ? 'selected' : '' }}>{{ $cuenta_gasto->co_cuenta_contable }} - {{ $cuenta_gasto->de_cuenta_contable }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="custom-select {!! $errors->has('cuenta_orden_pago') ? 'is-invalid' : '' !!}" name="cuenta_orden_pago" id="cuenta_orden_pago" {{ $errors->has('cuenta_orden_pago') ? 'aria-describedby="cuenta_orden_pago-error" aria-invalid="true"' : '' }}>
                                ...
                                <option value="{{ $cuenta_orden_pago->id }}" {{ $cuenta_orden_pago->id == old('cuenta_orden_pago') ? 'selected' : '' }}>{{ $cuenta_orden_pago->co_cuenta_contable }} - {{ $cuenta_orden_pago->de_cuenta_contable }}</option>
                                @endforeach
                            </select>
